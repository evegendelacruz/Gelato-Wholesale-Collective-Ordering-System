...
export default function AdminResetPasswordPage() {
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      // Query the admin_user table to verify email exists
      const { data, error } = await supabase
        .from('admin_user')
        .select('admin_auth_id, admin_email')
      ...
      const authIdValue = data.admin_auth_id;
    ...
    <div className="min-h-screen flex flex-col" style={{
      ...
      <header className="w-full py-4 px-8" style={{ backgroundColor: '#7A1F1F', color: '#FCF0E3' }}>
        <h1 className="font-normal text-4xl text-center" style={{ fontFamily: 'Egyptienne MN, serif'}}>
          Gelato Wholesale Collective - Admin
        </h1>
      </header>
